...
selpscca=function(Xdata1=Xdata1,Xdata2=Xdata2,mybetaold, myalphaold, tilderhoold,ncancorr,Ux,Sigma12r,Uy,Taux,Tauy,CovStructure){
  ...
  for(ii in 1:ncancorr){
    ...
    Alphai=CVXR::Variable(p)
    Objx=CVXR::norm1(Alphai)
    if(CovStructure=="Iden") {constraint=list(CVXR::norm_inf(Ux%*%Sigma12r%*%t(Uy)%*%mybetaold[,ii]- tilderhoold[ii]*Alphai) <= Taux[ii,1])
    }else if(CovStructure=="Ridge"){
    ...
    Objx=CVXR::norm1(Betai)
    if(CovStructure=="Iden") {constraint=list(CVXR::norm_inf(Uy%*%t(Sigma12r)%*%t(Ux)%*%myalphaold[,ii]- tilderhoold[ii]*Betai) <= Tauy[ii,1])
    }else if(CovStructure=="Ridge"){
    ...
    if((all(myalpha==0) || sum(sum(is.nan(myalpha)!=0))) || (all(mybeta==0)|| sum(sum(is.nan(mybeta)!=0)))){
      ...
      myrho=abs(stats::cor(Xdata1%*%myalpha, Xdata2%*%mybeta))
    }
  ...
  # myalphamat=qr.Q(qr(myalphamat))
  # mybetamat=qr.Q(qr(mybetamat))
